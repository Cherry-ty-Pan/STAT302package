...
```
After comparing the coeffiecients of different continents, we can tell that the `continent` has more influence on `lifeExp` than `gdpPercap`
...
for (i in 1:10) {
  test <- my_knn_cv(penguins[, 3:6], penguins$species, i, 5)
  train_err[i] <- sum(penguins$species != test[[1]]) / nrow(my_penguins)
  cv_err[i] <- test[[2]]
}
